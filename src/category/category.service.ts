...
export class CategoryService {
    ...
    async addCategory(body: CreateCategoryDto, userId: string) {

        const { name, logo } = body;

        const categoryExists = await this.categoryRepository.exists({ name, isDeleted: false });
    ...
    async getCategoryById(categoryId: string) {
        ...
        const category = await this.categoryRepository.findOneDocument({
            _id: categoryId,
            isDeleted: false
        });
    ...
    async updateCategory(categoryId: string, body: UpdateCategoryDto, userId: string) {
        ...
        try {
            ...
            if (uploadedFile?.public_id && category.logo?.public_id) {
